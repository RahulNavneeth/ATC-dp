/*
        File: ATC-dp_c.cpp
        Date and Time Created: 2024-06-23 19:00:10
...
void solve() {
	...
	for(int i = 1 ; i < N ; i++) {
		dp[i][0] = max({dp[i-1][0], dp[i-1][1] + a[i][0], dp[i-1][2] + a[i][0]});
		dp[i][1] = max({dp[i-1][1], dp[i-1][0] + a[i][1], dp[i-1][2] + a[i][1]});
		dp[i][2] = max({dp[i-1][2], dp[i-1][1] + a[i][2], dp[i-1][0] + a[i][2]});
	}
